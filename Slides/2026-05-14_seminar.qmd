...
![](../International_Migration_and_Nepal_Civil_War/Paper/Tables/Tables_Summary/2.Sample_Construction.html){width=100%}
...
For individual $i$ in district $j$ in birth cohort $k$:

$$
Y_{ijk} = \alpha + \beta_1 T_i + \beta_2 P_j + \beta_3 (T_i \times P_j) + \mathbf{X}_i'\gamma + \delta_d + \tau_a + \varepsilon_{ijk}
$$

- $T_i$ = 1 if individual was 0–17 at conflict onset (treatment cohort)
- $P_j$ = 1 if district above the conflict-intensity threshold
- $\delta_d$ = district fixed effects; $\tau_a$ = age fixed effects
- $\beta_3$ is the **DiD coefficient** of interest
...
![](../Paper/Tables/Tables_Summary/6.DiD_Two_Measures.html){width=100%}
...
![](../Paper/Tables/Tables_Main/7a.DiD_Regression.html){width=100%}
...
![](../Paper/Figures/Figures_Main/8a_intl_migrant_months_event_study.png){fig-align="center" width=85%}
